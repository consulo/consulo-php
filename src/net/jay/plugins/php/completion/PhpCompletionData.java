...
public class PhpCompletionData extends CompletionData {
    ...
    static {
        ...
        for (IElementType type : iElementTypes) {
            ...
            if (s.endsWith(keywordMarker)) s = s.substring(0, s.length() - keywordMarker.length());
            keywords[i++] = s;
        }
    }
...
//        System.out.println("--------");
        ...
        variant.addCompletion(new ContextGetter() {
            public Object[] get(PsiElement psiElement, CompletionContext completionContext) {
                ...
                if (psiElement.getNode().getElementType() == PHPTokenTypes.IDENTIFIER
                        && psiElement.getParent() instanceof Method
                        && ((Method) psiElement.getParent()).getNameNode() == psiElement.getNode()) {
                    return new Object[]{"__construct", "__destruct"};
                }
    ...
    public static final String[] superGlobals = new String[]{
            "_GET", "_POST", "_SERVER", "_FILES", "_COOKIE", "_SESSION",
            "_REQUEST", "_ENV", "GLOBALS"
    };
