    ...
    public boolean processDeclarations(@NotNull PsiScopeProcessor processor, @NotNull ResolveState state, PsiElement lastParent, @NotNull PsiElement source) {
        if (lastParent == null) {
            if (getStatement() != null
                    && !getStatement().processDeclarations(processor, state, null, source)) {
                return false;
            }
            if (getRepeatedExpression() != null
                    && !getRepeatedExpression().processDeclarations(processor, state, null, source)) {
                return false;
            }
            if (getConditionalExpression() != null
                    && !getConditionalExpression().processDeclarations(processor, state, null, source)) {
                return false;
            }
            if (getInitialExpression() != null
                    && !getInitialExpression().processDeclarations(processor, state, null, source)) {
                return false;
            }
