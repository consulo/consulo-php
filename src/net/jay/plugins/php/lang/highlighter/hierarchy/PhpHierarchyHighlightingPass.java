...
public class PhpHierarchyHighlightingPass extends TextEditorHighlightingPass
{
	...
	protected PhpHierarchyHighlightingPass(@NotNull Project project, @NotNull Editor editor, @NotNull PHPFile file)
	{
	...
	private void addOverridenGutterIfNeeded(PsiElement element)
	{
		if(element instanceof PhpClass)
		{
			...
			if(index != null)
			{
				...
				if(list.size() > 0)
				{
					markers.add(new PhpOverridenGutterInfo(element.getTextOffset(), element.getProject(), list, element));
				}
			}
		}
	}
	...
	private void addOverridesGutterIfNeeded(PsiElement element)
	{
		if(element instanceof Method && element.getParent() instanceof PhpClass)
		{
			...
			if(klass != null)
			{
				...
				if(methods.size() > 0)
				{
					markers.add(new PhpOverridesGutterInfo(element.getTextOffset(), element.getProject(), methods));
				}
			}
		}
	}
